...
export default function Profile() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
      <h1 className="font-display text-xl font-bold mb-6">My Profile</h1>
      ...
      <div className="rounded-2xl p-5" style={cardStyle}>
        <div className="flex justify-between items-center mb-3">
          ...
          <Link to="/buyer/orders" className="text-sm font-medium" style={{ color: 'var(--brand)' }}>View all →</Link>
        </div>

        {loadingOrders ? (
          <p className="text-sm" style={{ color: 'var(--ink-muted)' }}>Loading...</p>
        ) : orders.length === 0 ? (
          <p className="text-sm" style={{ color: 'var(--ink-muted)' }}>No orders yet.</p>
        ) : (
          <div className="space-y-2">
            {orders.map((order) => (
              <Link
                key={order._id} to={`/buyer/orders/${order._id}`}
                className="block text-sm p-2 rounded-lg" style={{ border: '1px solid var(--border)' }}
              >
                <span className="font-semibold tabular">₹{order.totalAmount}</span>
                <span style={{ color: 'var(--ink-muted)' }}> — {new Date(order.createdAt).toLocaleDateString()}</span>
              </Link>
            ))}
          </div>
        )}
